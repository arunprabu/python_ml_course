...
def create_feature_dict(
    ...
    engagement_score = min(
        100, (login_frequency_monthly / 30 * 50) + (features_used / 10 * 50)
    )
    ...
    # Data per login
    data_per_login = (
        data_consumption_gb / login_frequency_monthly
        if login_frequency_monthly > 0
        else 0
    )
    ...
    # Support rate annual
    support_rate_annual = (
        (support_tickets / tenure_months) * 12 if tenure_months > 0 else 0
    )
    ...
    plan_tenure_mismatch = (
        1 if (subscription_plan == "Basic" and tenure_months > 24) else 0
    )

    # Usage plan mismatch
    usage_plan_mismatch = (
        1 if (subscription_plan == "Basic" and features_used > 5) else 0
    )
    ...
    # Contract value risk
    contract_value_risk = (
        1 if (contract_type == "Monthly" and monthly_charges > 50) else 0
    )
    ...
    contract_tenure_ratio = (
        tenure_months / contract_tenure_ratio if contract_tenure_ratio > 0 else 0
    )
    ...
    engagement_growth_rate = (
        engagement_score / tenure_months if tenure_months > 0 else 0
    )
...
def main():
    # Header
    st.markdown(
        '<p class="main-header">🎯 Customer Churn Predictor</p>', unsafe_allow_html=True
    )
    st.markdown(
    ...
    if input_method == "Load Sample Customer" and sample_data is not None:
        ...
        if selected_customer_id:
            customer_data = sample_data[
                sample_data["customer_id"] == selected_customer_id
            ].iloc[0]
            default_values = {
                ...
                "login_frequency_monthly": int(
                    customer_data["login_frequency_monthly"]
                ),
            ...
            st.sidebar.info(
                f"Actual churn status: {'YES' if actual_churn == 1 else 'NO'}"
            )
    ...
    with st.sidebar.expander("📊 Demographics", expanded=True):
        ...
        tenure_months = st.number_input(
            "Tenure (months)", 1, 120, default_values["tenure_months"]
        )
    ...
    with st.sidebar.expander("💳 Subscription Details", expanded=True):
        subscription_plan = st.selectbox(
            ...
            index=["Basic", "Standard", "Premium"].index(
                default_values["subscription_plan"]
            ),
        )
        monthly_charges = st.number_input(
            "Monthly Charges ($)", 10.0, 200.0, default_values["monthly_charges"]
        )
        contract_type = st.selectbox(
            "Contract Type",
            ["Monthly", "Annual", "Two-Year"],
            index=["Monthly", "Annual", "Two-Year"].index(
                default_values["contract_type"]
            ),
        )
        payment_method = st.selectbox(
            "Payment Method",
            ["Credit Card", "Bank Transfer", "Digital Wallet"],
            index=["Credit Card", "Bank Transfer", "Digital Wallet"].index(
    ...
    with st.sidebar.expander("📈 Usage Metrics", expanded=True):
        login_frequency_monthly = st.number_input(
            ...
            0,
            100,
            default_values["login_frequency_monthly"],
        )
        features_used = st.slider(
            "Features Used", 1, 10, default_values["features_used"]
        )
        data_consumption_gb = st.number_input(
            "Data Consumption (GB/month)",
            0.0,
            200.0,
            default_values["data_consumption_gb"],
        )
        days_since_last_activity = st.number_input(
            "Days Since Last Activity",
            0,
            60,
            default_values["days_since_last_activity"],
        )
    ...
    with st.sidebar.expander("🔧 Support & Satisfaction", expanded=True):
        ...
        plan_changes = st.number_input(
            "Plan Changes", 0, 10, default_values["plan_changes"]
        )
        ...
        avg_resolution_hours = st.number_input(
            ...
            0.0,
            100.0,
            default_values["avg_resolution_hours"],
        )
    ...
    if predict_button:
        with st.spinner("Analyzing customer data..."):
            ...
            with col1:
                ...
                if prediction == 1:
                    ...
                    st.success(
                        f"**Retention Probability: {(1-churn_probability)*100:.1f}%**"
                    )
                    st.markdown("### 💡 Recommended Actions:")
                    st.markdown("""
                ...
                # Probability gauge
                st.plotly_chart(
                    plot_prediction_probability(churn_probability),
            ...
            with col2:
                ...
                if days_since_last_activity > 14:
                    risk_factors.append(
                        f"❌ Inactive for {days_since_last_activity} days"
                    )
            ...
            st.plotly_chart(
                plot_feature_importance(model, features_df), use_container_width=True
            )
        ...
        st.info(
            "👈 Configure customer details in the sidebar and click **Predict Churn** to see results"
        )
        ...
        with col2:
            ...
            st.markdown("""
            - **Accuracy**: See training script output
