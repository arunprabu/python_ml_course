...
def create_customer_value_features(df):
    ...
    df_featured["monthly_value_ratio"] = (
        df_featured["total_charges"] / df_featured["tenure_months"]
    )
    df_featured["monthly_value_ratio"] = df_featured["monthly_value_ratio"].replace(
        [np.inf, -np.inf], 0
    )
    print("✓ Created 'monthly_value_ratio' (ARPU)")

    # Charge efficiency (cost per feature)
    df_featured["charge_per_feature"] = (
        df_featured["monthly_charges"] / df_featured["features_used"]
    )
    df_featured["charge_per_feature"] = df_featured["charge_per_feature"].replace(
        [np.inf, -np.inf], 0
    )
...
def create_engagement_features(df):
    ...
    df_featured["engagement_velocity"] = (
        df_featured["engagement_score"] / df_featured["tenure_months"]
    )
    df_featured["engagement_velocity"] = df_featured["engagement_velocity"].replace(
        [np.inf, -np.inf], 0
    )
    ...
    # Data per login
    df_featured["data_per_login"] = (
        df_featured["data_consumption_gb"] / df_featured["login_frequency_monthly"]
    )
    df_featured["data_per_login"] = df_featured["data_per_login"].replace(
        [np.inf, -np.inf], 0
    )
    ...
    df_featured["features_utilization_rate"] = (
        df_featured["features_used"] / 10
    )  # Assuming max 10 features
    print("✓ Created 'features_utilization_rate'")

    # Data consumption per tenure
    df_featured["data_per_tenure"] = (
        df_featured["data_consumption_gb"] / df_featured["tenure_months"]
    )
...
def create_support_risk_features(df):
    ...
    df_featured["support_rate_annual"] = (
        df_featured["support_tickets"] / df_featured["tenure_months"]
    ) * 12
    print("✓ Created 'support_rate_annual'")

    # Resolution burden (total resolution time)
    df_featured["resolution_burden"] = (
        df_featured["avg_resolution_hours"] * df_featured["support_tickets"]
    )
    ...
    # Billing risk flag
    df_featured["billing_risk_flag"] = (df_featured["billing_issues_count"] > 0).astype(
        int
    )
    ...
    df_featured["complaint_ratio"] = (
        df_featured["billing_issues_count"] + df_featured["plan_changes"]
    ) / df_featured["tenure_months"]
    df_featured["complaint_ratio"] = df_featured["complaint_ratio"].replace(
        [np.inf, -np.inf], 0
    )
    print("✓ Created 'complaint_ratio'")

    # Support satisfaction ratio
    df_featured["support_satisfaction_ratio"] = df_featured["satisfaction_score"] / (
        df_featured["support_tickets"] + 1
    )
...
def create_interaction_features(df):
    ...
    df_featured["nps_satisfaction_alignment"] = abs(
        (df_featured["nps_score"] / 2) - df_featured["satisfaction_score"]
    )
...
def create_tenure_lifecycle_features(df):
    ...
    df_featured["engagement_growth_rate"] = df_featured["engagement_score"] / (
        df_featured["tenure_months"] + 1
    )
    print("✓ Created 'engagement_growth_rate'")

    # Tenure stability score (tenure normalized by age)
    df_featured["tenure_stability"] = df_featured["tenure_months"] / (
        df_featured["age"] * 12
    )
...
def save_feature_documentation(df, output_path):
    ...
    print(f"\nFeature Summary:")
    print(
        f"  - Original features: {len([f for f in features if f['type'] == 'Original'])}"
    )
    print(
        f"  - Engineered features: {len([f for f in features if f['type'] == 'Engineered'])}"
    )
