...
def generate_quality_report(df, stage="Initial"):
    ...
    for col in missing[missing > 0].index:
        print(f"  - {col}: {missing[col]} ({missing_percentage[col]}%)")
...
def clean_categorical_variables(df):
    ...
    if "gender" in df_clean.columns:
        ...
        # Map variations
        gender_map = {"M": "Male", "F": "Female", "Nan": np.nan}
    ...
    if "payment_method" in df_clean.columns:
        ...
        # normalizing the spaces between words in payment method
        df_clean["payment_method"] = df_clean["payment_method"].str.replace(
            r"\s+", " ", regex=True
        )
    ...
    if "subscription_plan" in df_clean.columns:
        ...
        df_clean["subscription_plan"] = (
            df_clean["subscription_plan"].astype(str).str.strip().str.title()
        )
    ...
    if "contract_type" in df_clean.columns:
        df_clean["contract_type"] = (
            df_clean["contract_type"].astype(str).str.strip().str.title()
        )
    ...
    for col in numerical_cols:
        if col in df_clean.columns:
            ...
            if missing_count > 0:
                ...
                print(
                    f"  - {col}: filled {missing_count} values with median ({median_value:.2f})"
                )
    ...
    for col in categorical_cols:
        if col in df_clean.columns:
            ...
            if missing_count > 0:
                ...
                print(
                    f"  - {col}: filled {missing_count} values with mode ('{mode_value}')"
                )
...
def correct_data_types(df):
    ...
    if "monthly_charges" in df_clean.columns:
        print(
            "Fixing 'monthly_charges' (removing $ symbols and converting to float)..."
        )
        df_clean["monthly_charges"] = (
            df_clean["monthly_charges"].astype(str).str.replace("$", "", regex=False)
        )
    ...
    if "total_charges" in df_clean.columns:
        ...
        df_clean["total_charges"] = pd.to_numeric(
            df_clean["total_charges"], errors="coerce"
        )
        print(f"  ✓ Converted to float")
    ...
    print(
        f"  ✓ Converted {len([c for c in numerical_cols_to_convert if c in df_clean.columns])} columns"
    )
    ...
    print(
        f"  ✓ Cleaned {len([c for c in string_columns if c in df_clean.columns])} columns"
    )
...
def fix_outliers_and_validate_ranges(df):
    ...
    if "monthly_charges" in df_clean.columns:
        ...
        median_val = df_clean[df_clean["monthly_charges"] > 0][
            "monthly_charges"
        ].median()
    ...
    if "engagement_score" in df_clean.columns:
        outliers = (
            (df_clean["engagement_score"] < 0) | (df_clean["engagement_score"] > 100)
        ).sum()
...
def validate_final_data(df):
    ...
    if "engagement_score" in df.columns:
        invalid_engagement = (
            (df["engagement_score"] < 0) | (df["engagement_score"] > 100)
        ).sum()
...
def main():
    ...
    df_clean = correct_data_types(
        df_clean
    )  # MUST convert types before handling missing values
